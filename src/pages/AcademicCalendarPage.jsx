...
export default function AcademicCalendarPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      ...
      <header
        className="relative bg-navy dark:bg-gray-900 text-white py-12 px-4 overflow-hidden"
        style={{
          backgroundImage: `
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {CALENDARS.map((cal) => (
            <div
              key={cal.id}
              className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 flex flex-col gap-4"
            >
              <div>
                <h2 className="font-display text-lg font-bold text-gray-900 dark:text-gray-100">{cal.title}</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{cal.description}</p>
              </div>
              <a
                href={cal.href}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 self-start bg-navy dark:bg-blue-700 hover:bg-navy/90 dark:hover:bg-blue-600 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors mt-auto"
              >
                Open Calendar
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400 hover:text-navy dark:hover:text-blue-400 transition-colors mt-6"
        >
